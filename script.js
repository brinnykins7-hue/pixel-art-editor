...
window.History = (function () {
  ...
  function restoreState(snapshot) {
    ...
    // Restore active indices with strict boundaries / fallbacks
    var activeIdx = snapshot.activeLayerIndex !== undefined ? snapshot.activeLayerIndex : 0;
    var activeFrameIdx = snapshot.activeFrameIndex !== undefined ? snapshot.activeFrameIndex : 0;

    // Number of layers may have changed — rebuild the array
    ...
    State.activeLayerIndex = Math.max(0, Math.min(State.layers.length - 1, activeIdx));

    if (State.frames && State.frames.length > 0) {
      State.activeFrameIndex = Math.max(0, Math.min(State.frames.length - 1, activeFrameIdx));
    } else {
      State.activeFrameIndex = 0;
    }
