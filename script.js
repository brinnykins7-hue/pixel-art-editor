...
window.Canvas = (function () {
  ...
  function init() {
    ...
    EventBus.on('playbackRender', (canvas) => {
  ...
  function render() {
    ...
    if (State.playing && playbackCanvas) {
      compCtx.drawImage(playbackCanvas, 0, 0);
    } else {
  ...
  function compositeFrame(frameIndex) {
    ...
    if (frameIndex === State.activeFrameIndex) {
      ...
      for (let i = 0; i < layersData.length; i++) {
        ...
        // Layer data stores an ImageData, a canvas reference, or a dataURL
        if (ld.imageData) {
...
window.Palette = (function () {
  ...
  function setBackgroundColor(rgba) {
    ...
    elBg.style.backgroundColor = ColorUtil.rgbaToCSS(State.backgroundColor);
    EventBus.emit('colorChanged');
  }
  ...
  function updateUI() {
    ...
    /* Swatches */
    elFg.style.backgroundColor = ColorUtil.rgbaToCSS(c);
    elFg.style.backgroundImage = c.a < 255 ? '' : 'none';
    elBg.style.backgroundColor = ColorUtil.rgbaToCSS(State.backgroundColor);
    elBg.style.backgroundImage = State.backgroundColor.a < 255 ? '' : 'none';
...
window.Animation = (function () {
  ...
  function _playbackLoop(timestamp) {
    ...
    /* Duration for this frame: use FPS slider speed directly */
    var frameDuration = 1000 / State.fps;
...
window.Export = (function () {
  ...
  window.doConfirmImport = function () {
    ...
    reader.onload = function (e) {
      var img = new Image();
      img.onload = function () {
        if (selectedImportMode === 'sheet') {
          var sliceW = parseInt(document.getElementById('import-slice-w').value) || 32;
          var sliceH = parseInt(document.getElementById('import-slice-h').value) || 32;
          var dest = document.getElementById('import-slice-dest').value || 'frames';
          sliceAndImportSpriteSheet(img, file, sliceW, sliceH, dest);
        } else {
          importImageAsLayer(img, file);
        }
      };
  ...
  function loadGifuct(callback) {
    if (window.gifuct) {
      callback();
      return;
    }
    if (typeof UI !== 'undefined' && UI.showToast) {
      UI.showToast('Loading GIF decoder...', 'info');
    }
    ...
    script.onload = function () {
      if (window.gifuct) {
        ...
        if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Failed to initialize GIF decoder', 'error');
      }
    };
    script.onerror = function () {
      if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Failed to load GIF decoder library', 'error');
    };
    ...
    loadGifuct(function () {
      ...
      reader.onload = function (e) {
        try {
          ...
          var parsedGif = window.gifuct.parseGIF(arrayBuffer);
          var decompressed = window.gifuct.decompressFrames(parsedGif, true);

          if (!decompressed || decompressed.length === 0) {
            if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('No frames found in GIF', 'error');
            return;
          }

          var width = parsedGif.lsd.width || decompressed[0].dims.width;
          var height = parsedGif.lsd.height || decompressed[0].dims.height;

          var doReplace = confirm(
            'Import GIF: Found ' + decompressed.length + ' frames (' + width + '×' + height + ').\n\n' +
            'Replace current animation frames?'
          );

          var newFrames = [];
          var tempCanvas = CanvasUtil.createCanvas(width, height);

          for (var i = 0; i < decompressed.length; i++) {
            var frame = decompressed[i];

            if (frame.disposalType === 2) {
              tempCanvas.ctx.clearRect(0, 0, width, height);
            }

            var patchCanvas = CanvasUtil.createCanvas(frame.dims.width, frame.dims.height);
            var imgData = new ImageData(frame.patch, frame.dims.width, frame.dims.height);
            patchCanvas.ctx.putImageData(imgData, 0, 0);

            tempCanvas.ctx.drawImage(patchCanvas.canvas, frame.dims.left, frame.dims.top);

            var finalDataURL = tempCanvas.canvas.toDataURL();
          ...
          if (doReplace) {
            ...
            State.activeFrameIndex = 0;
            if (typeof Animation !== 'undefined') Animation.loadFrame(0);
          } else {
            for (var j = 0; j < newFrames.length; j++) {
              State.frames.push(newFrames[j]);
            }
            State.activeFrameIndex = State.frames.length - newFrames.length;
            if (typeof Animation !== 'undefined') Animation.loadFrame(State.activeFrameIndex);
          }

          if (typeof History !== 'undefined') History.pushState('Import GIF');
          EventBus.emit('frameChanged');
          EventBus.emit('render');
          if (typeof Canvas !== 'undefined') Canvas.zoomToFit();
          if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Imported ' + decompressed.length + ' frames from GIF', 'success');

        } catch (err) {
          console.error(err);
          if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Failed to parse GIF file', 'error');
        }
      };
  ...
  function importImageAsLayer(img, file) {
    var needsResize = (img.width > State.width || img.height > State.height);
    if (needsResize) {
      var doResize = confirm(
        'Image (' + img.width + '×' + img.height + ') is larger than the canvas (' +
  ...
  function sliceAndImportSpriteSheet(img, file, sliceW, sliceH, dest) {
    var cols = Math.floor(img.width / sliceW);
    var rows = Math.floor(img.height / sliceH);
    var totalTiles = cols * rows;

    if (totalTiles === 0) {
      if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Invalid slice dimensions: larger than image', 'error');
      return;
    }

    var doImport = confirm(
      'Sprite Sheet Import: Found ' + totalTiles + ' frames (' + cols + ' columns × ' + rows + ' rows).\n\n' +
      'Slice and import?'
    );
    if (!doImport) return;
    ...
    if (dest === 'frames') {
      ...
      for (var r = 0; r < rows; r++) {
        for (var c = 0; c < cols; c++) {
          ...
          var tile = CanvasUtil.createCanvas(sliceW, sliceH);
          tile.ctx.drawImage(img, x, y, sliceW, sliceH, 0, 0, sliceW, sliceH);
      ...
      var replaceCurrent = confirm('Replace current animation frames with the sliced sprite sheet?');
      if (replaceCurrent) {
        State.frames = newFrames;
        State.activeFrameIndex = 0;
        if (typeof Animation !== 'undefined') Animation.loadFrame(0);
      ...
      for (var r = 0; r < rows; r++) {
        for (var c = 0; c < cols; c++) {
          ...
          var layer = {
            id:        State.nextLayerId(),
            name:      (file.name.replace(/\.[^.]+$/, '') || 'Import') + '_' + (r * cols + c + 1),
            canvas:    tile.canvas,
            ctx:       tile.ctx,
            visible:   true,
            opacity:   100,
    ...
    if (typeof History !== 'undefined') History.pushState('Import Sprite Sheet');
    EventBus.emit('frameChanged');
    EventBus.emit('render');
    if (typeof Canvas !== 'undefined') Canvas.zoomToFit();
    if (typeof UI !== 'undefined' && UI.showToast) UI.showToast('Imported ' + totalTiles + ' slices from sprite sheet', 'success');
  }
  ...
  function importImage(file) {
    ...
    var html = `
      <div class="import-options">
        <p style="font-size: 12px; color: var(--text-secondary); margin-bottom: 12px;">Choose how you want to import <strong>${file.name}</strong>:</p>
        <div style="margin-bottom: 15px; display: flex; gap: 8px;">
          <button id="import-mode-layer" class="modal-btn modal-btn-primary" style="flex: 1;" onclick="document.getElementById('sprite-sheet-settings').style.display='none'; this.classList.add('modal-btn-primary'); this.classList.remove('modal-btn-secondary'); document.getElementById('import-mode-sheet').classList.remove('modal-btn-primary'); document.getElementById('import-mode-sheet').classList.add('modal-btn-secondary'); selectImportMode('layer');">Single Layer</button>
          <button id="import-mode-sheet" class="modal-btn modal-btn-secondary" style="flex: 1;" onclick="document.getElementById('sprite-sheet-settings').style.display='block'; this.classList.add('modal-btn-primary'); this.classList.remove('modal-btn-secondary'); document.getElementById('import-mode-layer').classList.remove('modal-btn-primary'); document.getElementById('import-mode-layer').classList.add('modal-btn-secondary'); selectImportMode('sheet');">Sprite Sheet</button>
        </div>
        
        <div id="sprite-sheet-settings" style="display: none; border: 1px solid var(--border); padding: 12px; border-radius: var(--radius-md); background: var(--bg-darkest); margin-bottom: 15px;">
          <div style="display: flex; gap: 8px; margin-bottom: 10px;">
            <div style="flex: 1;">
              <label style="font-size: 11px; color: var(--text-secondary); margin-bottom: 4px; display: block;">Frame Width (px)</label>
              <input type="number" id="import-slice-w" value="32" min="1" style="width: 100%; padding: 4px 8px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-input); color: var(--text);">
            </div>
            <div style="flex: 1;">
              <label style="font-size: 11px; color: var(--text-secondary); margin-bottom: 4px; display: block;">Frame Height (px)</label>
              <input type="number" id="import-slice-h" value="32" min="1" style="width: 100%; padding: 4px 8px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-input); color: var(--text);">
            </div>
          </div>
          
          <div>
            <label style="font-size: 11px; color: var(--text-secondary); margin-bottom: 4px; display: block;">Import As</label>
            <select id="import-slice-dest" style="width: 100%; padding: 4px 8px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-input); color: var(--text); font-size: 12px;">
              <option value="frames">New Animation Frames</option>
              <option value="layers">New Layers in Active Frame</option>
            </select>
          </div>
        </div>
      </div>
    ...
    var footer = `
      ...
      <button class="modal-btn modal-btn-primary" onclick="doConfirmImport()">Import</button>
    `;

    UI.showModal('Import Image Options', html, footer);
  }
  ...
  GIFEncoder.prototype.writeGraphicControlExt = function (/* indexedPixels */) {
    ...
    // packed: reserved(3)=0, disposal(3)=2(restore to background color), user input=0, transparency=1
    // 0_010_0_1 = 0x09
    var disposal = 2;              // restore to background color (erases frame before drawing next)
...
window.Shortcuts = (function () {
  ...
  function selectTool(toolName) {
    if (typeof Tools !== 'undefined' && Tools.setTool) {
...
const UI = {
  ...
  showTemplateModal() {
    ...
    list.forEach(t => {
      let icon = '';
      if (t.name.includes('Blank')) {
        icon = `<svg viewBox="0 0 16 16" width="32" height="32" style="color: var(--text-dim);"><rect x="2" y="2" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.5" stroke-dasharray="2 1"/><rect x="4" y="4" width="8" height="8" fill="none" stroke="currentColor" stroke-width="1"/></svg>`;
      } else if (t.name.includes('Character')) {
        icon = `<svg viewBox="0 0 16 16" width="32" height="32" style="color: var(--accent);"><circle cx="8" cy="5" r="3.5" fill="none" stroke="currentColor" stroke-width="1.5"/><path d="M3 14a5 5 0 0110 0" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>`;
      } else if (t.name.includes('RPG') || t.name.includes('Top-Down')) {
        icon = `<svg viewBox="0 0 16 16" width="32" height="32" style="color: #ffb13b;"><rect x="2" y="4" width="12" height="8" rx="1" fill="none" stroke="currentColor" stroke-width="1.5"/><line x1="8" y1="4" x2="8" y2="12" stroke="currentColor" stroke-width="1.5"/><line x1="2" y1="8" x2="14" y2="8" stroke="currentColor" stroke-width="1.5"/></svg>`;
      } else {
        icon = `<svg viewBox="0 0 16 16" width="32" height="32" style="color: var(--text);"><rect x="1" y="1" width="6" height="6" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="9" y="1" width="6" height="6" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="1" y="9" width="6" height="6" fill="none" stroke="currentColor" stroke-width="1.5"/><rect x="9" y="9" width="6" height="6" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>`;
      }
      html += `
        <div class="template-card" onclick="UI.loadTemplate('${t.name}')">
          <div class="template-preview" style="display:flex; align-items:center; justify-content:center;">${icon}</div>
          <div class="template-name">${t.name}</div>
          <div class="template-size">${t.width}×${t.height}</div>
        </div>
      `;
    });
    html += '</div>';
  ...
  setupThemeToggle() {
    ...
    const isLightTheme = savedTheme === 'light' || (!savedTheme && !systemPrefersDark);
  ...
  updateThemeIcon(isLight) {
    ...
    if (!iconPath) return;
    if (isLight) {
      iconPath.setAttribute('d', 'M12.3 22h-.1c-5.5 0-10-4.5-10-10 0-4.8 3.5-9 8.3-9.9.5-.1.9.3.8.8-.1.5-.4.9-.7 1.4-1 1.7-1.4 3.7-1.4 5.7 0 5 4 9 9 9 1.6 0 3.2-.4 4.6-1.2.4-.2.9 0 1 .4.2.5-.1 1-.6 1.2-1.9 1-4 1.6-6 1.6z');
    } else {
      iconPath.setAttribute('d', 'M12 7a5 5 0 110 10 5 5 0 010-10zm0 2a3 3 0 100 6 3 3 0 000-6zm0-7a1 1 0 011 1v2a1 1 0 11-2 0V3a1 1 0 011-1zm0 15a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1zM5.22 5.22a1 1 0 011.41 0l1.42 1.42a1 1 0 11-1.42 1.42L5.22 6.64a1 1 0 010-1.41zm10.72 10.72a1 1 0 011.41 0l1.42 1.42a1 1 0 11-1.42 1.42l-1.42-1.42a1 1 0 010-1.41zM2 12a1 1 0 011-1h2a1 1 0 110 2H3a1 1 0 01-1-1zm15 0a1 1 0 011-1h2a1 1 0 110 2h-2a1 1 0 01-1-1zM6.64 17.36a1 1 0 010 1.42l-1.42 1.42a1 1 0 11-1.42-1.42l1.42-1.42a1 1 0 011.42 0zm10.72-10.72a1 1 0 010 1.42l-1.42 1.42a1 1 0 11-1.42-1.42l1.42-1.42a1 1 0 011.42 0z');
    }
  },
...
const App = {
  ...
  async checkAutoSave() {
    ...
    // No auto-save found, show welcome dialog so user can easily choose templates, open existing, etc.
