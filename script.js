...
window.Canvas = (function () {
  ...
  function _applyPan() {
    if (!wrapper) return;
    // Center the canvas wrapper in the viewport, then apply pan offset
    const vpW = viewport.clientWidth;
    const vpH = viewport.clientHeight;
    const dw = displayCanvas.width;
    const dh = displayCanvas.height;

    const cx = Math.round((vpW - dw) / 2 + State.panX);
    const cy = Math.round((vpH - dh) / 2 + State.panY);
  ...
  function compositeFrame(frameIndex) {
    ...
    if (frameIndex === State.activeFrameIndex) {
      ...
      for (let i = 0; i < layersData.length; i++) {
        ...
        ctx.save();
        ctx.globalAlpha = ld.opacity !== undefined ? ld.opacity : 1;
...
window.History = (function () {
  ...
  function restoreState(snapshot) {
    ...
    // Clamp activeLayerIndex just in case
    if (State.activeLayerIndex >= State.layers.length) {
      State.activeLayerIndex = Math.max(0, State.layers.length - 1);
    }
...
window.Templates = (function () {
  ...
  function load(templateName) {
    ...
    // Clear history for the fresh project
